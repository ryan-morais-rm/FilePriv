    ...
    tbody.innerHTML = Object.entries(nodesData).map(([id, node]) => `
        <tr id="row-${id}">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button onclick="window.toggleEdit(${id})" class="text-primary-custom hover:text-blue-800 font-bold bg-blue-50 px-3 py-1 rounded border border-blue-200 hover:bg-blue-100 transition">Editar</button>
            </td>
        </tr>
    `).join('');
}
...
function toggleEdit(id) {
    ...
    if (btn.innerText === 'Editar') {
        ...
        btn.classList.remove('bg-blue-50', 'text-primary-custom', 'border-blue-200', 'hover:bg-blue-100');
        btn.classList.add('bg-green-100', 'text-green-700', 'border-green-300', 'hover:bg-green-200');
    } else {
        ...
        nodesData[id].ip = newIp;
        nodesData[id].port = newPort;
        nodesData[id].cpu = newCpu;
        nodesData[id].ram = newRam;
        nodesData[id].disk = newDisk;

        updateChartLabels();

        btn.innerText = 'Editar';
        btn.classList.add('bg-blue-50', 'text-primary-custom', 'border-blue-200', 'hover:bg-blue-100');
        btn.classList.remove('bg-green-100', 'text-green-700', 'border-green-300', 'hover:bg-green-200');
    }
}
...
function initChart() {
    ...
    if (typeof Chart !== 'undefined') {
        ...
        console.warn("Chart.js não foi carregado na página.");
    }
}
